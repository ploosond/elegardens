  ...
  function processNode(node: any): string {
    ...
    switch (nodeType) {
      ...
      case 'text':
        ...
        if (node.format) {
          const format =
            typeof node.format === 'number' ? node.format : parseInt(node.format, 10) || 0
      ...
      case 'list':
        ...
        return result ? `<${listTag} class="${listClass}" ${listStyle}>${result}</${listTag}>` : ''
      case 'listitem':
        return result ? `<li class="mb-1" style="display: list-item;">${result}</li>` : ''
      case 'link':
