...
export default function AboutPage() {
  ...
  useEffect(() => {
    setLoading(true)
    setError(null)
    fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_URL}/api/globals/about?locale=${locale}`)
      .then((res) => {
  ...
  const get = (base: string) => about?.[`${base}_${locale}`] || about?.[`${base}_en`] || ''
    ...
    <div>
      ...
      <section className="py-8 sm:py-10 md:py-12">
        <div className="mx-auto max-w-5xl px-4 text-center sm:px-6">
          <h2 className="mb-4 text-3xl font-bold text-primary md:text-4xl">{get('roots_title')}</h2>
          <p className="mb-8 text-base text-text sm:text-lg md:text-xl">{get('roots_intro')}</p>
            ...
            <div className="order-2 flex items-center px-2 sm:px-4 md:order-1 md:min-h-[20rem] md:px-0 lg:min-h-[24rem]">
              <div className="w-full px-4 py-4 sm:px-8 sm:py-6 md:px-0">
                ...
                <p className="mb-4 text-justify text-base text-text">
                  <span className="font-bold text-text">{get('mission_title')}</span>{' '}
                  {get('mission_desc')}
                </p>
                <p className="mb-4 text-justify text-base text-text">
                  <span className="font-bold text-text">{get('vision_title')}</span>{' '}
                  {get('vision_desc')}
                </p>
                <p className="text-justify text-base text-text">
                  <span className="font-bold">{get('values_title')}</span> {get('values_desc')}
                </p>
              </div>
            </div>
            {/* CEO Image */}
            <div className="relative order-1 min-h-[20rem] overflow-hidden rounded-md md:order-2 lg:min-h-[24rem]">
              {about?.ceo_image && about.ceo_image.url && (
                <Image
