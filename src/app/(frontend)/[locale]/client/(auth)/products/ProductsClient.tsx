...
export default function ProductsClient({ initialProducts }: ProductsClientProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered
  }, [debouncedProductIdSearch, debouncedProductNameSearch, availabilityFilter, initialProducts])

  const totalItems = getTotalItems()
  const totalProducts = getTotalProducts()

  const hasActiveFilters = productIdSearch || productNameSearch || availabilityFilter !== 'all'
      ...
      <div className="mb-6 rounded-lg border border-primary/20 bg-primary/5 p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <span className="text-text/70">{tB2B('total_products', { count: totalProducts })}</span>
          </div>
          {totalItems > 0 && (
            <Button
              onClick={() => {
      ...
      <div className="mb-6 rounded-lg border border-muted bg-bg p-4">
        ...
        {hasActiveFilters && (
          <div className="mt-4 flex justify-end">
            <Button onClick={clearFilters} variant="secondary" className="whitespace-nowrap">
              {tB2B('clear_filters')}
            </Button>
          </div>
        )}
